...
class StructuredDataPhantom extends Property
{
    ...
    public function __construct( 
    	stdClass $sd, AssetOperationHandlerService $service, $data_definition_id=NULL )
    {
        ...
        if( $sd != NULL )
        {
            ...
            else
            {
                ...
                if( self::DEBUG ) { DebugUtility::out( "Number of nodes in std: " . count( $child_nodes ) ); }
            }
            // convert stdClass to objects
            StructuredDataNodePhantom::processStructuredDataNodePhantoms( 
        ...
        if( self::DEBUG ) { DebugUtility::out( "First node ID: " . $first_node_id ); }
    }
    ...
    public function appendSibling( $first_node_id )
    {
        ...
        {
        	if( self::DEBUG ) { DebugUtility::out( "non_ambiguous" ); }
    ...
    {
        if( self::DEBUG ) { DebugUtility::out( "Node ID: " . $node_name ); }
        $par_id     = $this->node_map[ $node_name ]->getParentId();
        if( self::DEBUG ) { DebugUtility::out( "Parent ID: " . $par_id ); }
    ...
    {
    	if( self::DEBUG ) { DebugUtility::dump( $block ); }
    	if( self::DEBUG ) { DebugUtility::out( $node_name ); }
    ...
    public function swapData( $node_name1, $node_name2 )
    {
            ...
            StructuredDataNodePhantom::removeLastIndex( $node_name2 ) )
        {
            throw new NodeException( 
            	"The nodes $node_name1 and $node_name2 are not siblings." );
        }
        ...
        if( self::DEBUG ) { DebugUtility::out( "Sibling count: $sibling_count" ); }
        
        for( $i = 0; $i < $sibling_count; $i++ )
        {
        	if( self::DEBUG ) { DebugUtility::out( $siblings[ $i ]->getIdentifier() ); }
            
            // find the two positions
            if( $siblings[ $i ]->getIdentifier() == $node_name1 )
            {
                $node_pos1 = $i;
                if( self::DEBUG ) { DebugUtility::out( "Node 1 position: $node_pos1" ); }
            }
            if( $siblings[ $i ]->getIdentifier() == $node_name2 )
            {
                $node_pos2 = $i;
                if( self::DEBUG ) { DebugUtility::out( "Node 2 position: $node_pos2" . BR ); }
            }
        }
    ...
    {
		if( self::DEBUG ) { DebugUtility::out( $field_name ); }
