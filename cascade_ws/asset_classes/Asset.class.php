...
abstract class Asset
{
    ...
    public function __construct( AssetOperationHandlerService $service, stdClass $identifier )
    {
        ...
        if( self::DEBUG && self::DUMP ) { DebugUtility::dump( $identifier ); }
    ...
    public function copy( Container $parent, $new_name )
    {
    	if( $new_name == "" )
    	{
    		throw new EmptyNameException( M::EMPTY_NAME );
    	}
    ...
    public function getAudits( $type="", DateTime $start_time=NULL, DateTime $end_time=NULL )
    {
        ...
        {
            if( self::DEBUG && !is_string( $type ) ) { DebugUtility::out( "Not a string" ); }
        ...
        if( $start_time != NULL )
        {
            if( $end_time != NULL )
            {
                if( $end_time < $start_time )
                    throw new Exception( M::SMALLER_END_TIME );
        ...
        if( $service->isSuccessful() )
        {
            if( self::DEBUG ) { DebugUtility::dump( $service->getAudits() ); }
            ...
            if( $count > 0 )
            {
				...
				{
					if( self::DEBUG && self::DUMP ) { DebugUtility::dump( $audit_std ); }
    ...
    {
        if( self::DEBUG ) { DebugUtility::out( "From Asset::getPropertyName " . $this->property_name ); }
    ...
    public function getSubscribers()
    {
        ...
        {
            if( self::DEBUG ) { DebugUtility::out( "Successfully listing subscribers" ); }
            $results = array();
    ...
    public function publishSubscribers( Destination $destination=NULL )
    {
        ...
        if( $subscriber_ids != NULL )
        {
            ...
            {
                if( self::DEBUG ) { DebugUtility::out( "Publishing " . $subscriber_id->getId() ); }
    ...
    public static function getAsset( $service, $type, $id_path, $site_name=NULL )
    {
        ...
        {
        	if( self::DEBUG ) { DebugUtility::out( $e->getMessage() ); }
        	throw $e;
        }
    }
