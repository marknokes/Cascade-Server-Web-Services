...
class DataDefinition extends ContainedAsset
{
    ...
    */
    ...
    {
    	if( self::DEBUG ) { DebugUtility::out( "$type, $identifier" ); }
    	...
    	if( $type == "group" )
    	{
    		...
    		if( $more_than_one )
    		{
				...
				foreach( $xml_element->children() as $child )
				{
    				...
    				if( self::DEBUG ) { DebugUtility::out( "Child type in group: $child_type" ); }
    ...
    private function createChildStd( $child, $child_type, $child_identifier )
    {
		...
		if( isset( $grandchild ) )
		{
			...
			{
				$child_std->text = StructuredDataNode::CHECKBOX_PREFIX;
			}
			else if( $grandchild_type == "selector-item" )
			{
				$child_std->text = StructuredDataNode::SELECTOR_PREFIX;
			}
		}
