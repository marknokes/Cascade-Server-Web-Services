...
class Site extends ScheduledPublishing
{
    ...
    const NEVER    = T::NEVER;
    const ONE      = T::ONE;
    const FIFTEEN  = T::FIFTEEN;
    const THIRTY   = T::THIRTY;
    ...
    {
        if( $r == NULL )
            throw new NullAssetException( "The role cannot be NULL." );
        ...
        $this->role_assignments[] = new RoleAssignment( $ra );
    ...
    public function edit()
    {
        ...
        if( $site->usesScheduledPublishing ) // publishing is scheduled
        {
            ...
            if( $site->publishIntervalHours == NULL )
                unset( $site->publishIntervalHours );
                
            if( $site->publishDaysOfWeek == NULL )
                unset( $site->publishDaysOfWeek );
                
            if( $site->cronExpression == NULL )
        ...
        if( self::DEBUG && self::DUMP ) { DebugUtility::dump( $site ); }
    ...
    public function setCssClasses( $classes )
    {
        ...
        {
            throw new NullAssetException( "The CSS file must be set first." );
        }
    ...
    public function setDefaultMetadataSet( MetadataSet $m )
    {
        if( $m == NULL )
        {
            throw new NullAssetException( "The metadata set cannot be NULL." );
        }
    ...
    public function setRecycleBinExpiration( $e )
    {
        ...
        {
            throw new UnacceptableValueException( "Unacceptable value: $e" );
        }
    ...
    private function processRoleAssignments()
    {
        ...
        if( isset( $ra ) )
        {
            ...
            {
                $this->role_assignments[] = new RoleAssignment( $role_assignment );
            }
        }
    }
