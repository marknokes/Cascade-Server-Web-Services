...
class Message
{
    ...
    public function __construct( stdClass $message )
    {
        if( $message != NULL )
        {
            ...
            if( StringUtility::startsWith( $this->subject, self::TYPE_EXPIRATION ) )
            {
                //echo "Type " . self::TYPE_EXPIRATION . BR;
                $this->type = self::TYPE_EXPIRATION;
            }
            else if( StringUtility::startsWith( $this->subject, self::TYPE_PUBLISH ) )
            {
                //echo "Type " . self::TYPE_PUBLISH . BR;
                $this->type = self::TYPE_PUBLISH;
            }
            else if( StringUtility::startsWith( $this->subject, self::TYPE_UNPUBLISH ) )
            {
                //echo "Type " . self::TYPE_UNPUBLISH . BR;
                $this->type = self::TYPE_UNPUBLISH;
            }
            else if( StringUtility::startsWith( $this->subject, self::TYPE_SUMMARY ) )
            {
                //echo "Type " . self::TYPE_SUMMARY . BR;
                $this->type = self::TYPE_SUMMARY;
            }
            else if( StringUtility::startsWith( $this->subject, self::TYPE_WORKFLOW ) )
            {
    ...
    public function display()
    {
        echo S_H2      . "Message " . $this->id . E_H2 .
            ...
            L::TO      . $this->to . BR .
            L::FROM    . $this->from . BR .
            L::SUBJECT . $this->subject . BR .
            L::BODY    . BR . $this->body . BR .
            L::DATE    . date_format( $this->date, 'Y-m-d H:i:s' ) . BR . HR;
            HR;
        return $this;
    }
            ...
            foreach( $this->msg_errors as $error )
            {
                try
                {
                    list( $destination, $site, $path ) = 
                        StringUtility::getExplodedStringArray( "\t", $error );
