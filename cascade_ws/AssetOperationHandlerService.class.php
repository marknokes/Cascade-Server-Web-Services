...
class AssetOperationHandlerService
{
    ...
    *
    * @throws ServerException if the server connection can be established
    */
    ...
    * @param string $id_string The 32-digit id string
    ...
    * @param stdClass $actionIdentifier The identifier of the action
    * @param string   $transitionComment The comments
    */
    public function performWorkflowTransition( 
        $workflowId, stdClass $actionIdentifier, $transitionComment='' )
    {
    ...
    */
    ...
    {
    	if( self::DEBUG ) { DebugUtility::dump( $identifier ); }
    ...
    */
    public function createId( $type, $id_path, $site_name = NULL )
    {
    	if( !( is_string( $type ) && ( is_string( $id_path ) || is_int( $id_path ) ) ) )
    		throw new UnacceptableValueException( "Only strings are accepted in createId." );
        ...
        else if( StringUtility::startsWith( $id_path, "ROOT_" ) )
        {
        	$identifier->id = $id_path;
        }
        // asset in Global
    ...
    */
    public function getAsset( $type, $id_path, $site_name=NULL )
    {
        ...
        {
        	if( self::DEBUG && self::DUMP ) { DebugUtility::out( $e->getMessage() ); }
        	throw $e;
        }        
    }
